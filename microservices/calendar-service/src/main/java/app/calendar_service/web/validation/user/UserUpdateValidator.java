...
public class UserUpdateValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        userService.findActiveUserById(user.getId()).ifPresentOrElse( foundUser -> {

            if(!foundUser.getUsername().equals(user.getUsername()) && userService.existByUsername(user.getUsername())){
                errors.rejectValue("username", "" ,"ya ha sido registrado");
            }

            if(!foundUser.getEmail().equals(user.getEmail()) && userService.existsByEmail(user.getEmail())){
                errors.rejectValue("email", "" ,"ya ha sido registrado");
            }

        }, () -> {

            errors.rejectValue("usuario", "" ,"inválido");
